...
2026-09-17补充：用户已提供目标仓库 https://github.com/zjugxz/WiNet-Lab ，要求了解将现有初版提前上线的步骤。新目标替代“整站验收后才首次上传”的历史时点限制。操作流程及本轮验证见 [初版发布指南](github-pages-preview.md)。因外部连接失败，未核实仓库是否为空、是否公开或Pages状态；本轮未上传或启用Pages。下文尚未取得仓库链接等为历史记录。
