...
- 用户现明确要求开展 Publications，参考 https://marslab.tech/publications/ 的页面实现；论文来源已选择由用户自行提供收录清单，不自动导入老师主页或参考站论文。清单尚未收到，当前已核对参考站布局/筛选并整理资料字段，页面仍是占位路由。详见 publications.md。
...
- 使用本地 Git。用户于2026-09-17提出将现有初版提前发布到 zjugxz/WiNet-Lab，新指示替代此前整体验收后才首次上传的时点限制；随后授权补齐本地部署配置，现已实现并验证，未执行推送或启用Pages，不代表整站验收。
- 实时维护项目文档，方便后续 agent 接手。

## 托管与账号（2026-09-15 变更）

2026-09-17最新补充：用户提供 https://github.com/zjugxz/WiNet-Lab ，作为初版发布目标，并随后授权补齐本地部署配置。预期默认项目地址为 https://zjugxz.github.io/WiNet-Lab/ 。配置及工作流已实现，本地验证通过；仓库可见性、是否为空、权限和Pages状态仍未独立核验。详见 [发布步骤](github-pages-preview.md)，尚未实施远程操作。下列未提供仓库链接及禁止提前上传等内容是历史状态，以此补充为准。
