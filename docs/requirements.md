# 已确认需求

更新日期：2026-09-15（Asia/Shanghai）。

## 网站

- 名称：Winet Group；大小写等品牌规范尚待后续设计确认。
- 参考网站：https://marslab.tech/ 。已读取五个栏目的网页内容；实际桌面、手机视觉与交互尚未逐项验证。
- 页面：Home、Research、Publications、People、Contact。
- 页面布局参考该网站；本课题组的具体文案、研究方向、成员、论文、图片和联系信息尚待提供及确认。
- 已确认使用在线内容管理后台，访问体验兼顾中国大陆与国际访客。
- 共享组件、独立页面、统一内容数据源；日常内容修改不应要求深入编辑代码。

## 协作与交付

- 遵循根目录 `AGENTS.md` 中六条用户原则。
- 每个子任务验证后等待用户验收，再进入下一子任务。
- 使用本地 Git；整个项目完成并经用户验收前，不上传 GitHub。
- 实时维护项目文档，方便后续 agent 接手。

## 域名

- 用户同意先开展域名筛选与购买指导。
- 预算偏好：名称优先，价格出来后再决定；没有固定金额上限，也未授权付款。
- 主域名、注册商、注册主体、注册年限均未选定。
- 用户追加提出 `winetgroup.tech` 作为候选进行比较；这不是购买决定。
- 用户随后提供查询截图：`winetgroup.tech` 首年人民币 8 元，续费 259 元/年。尚未确认接受该费用或授权购买。
- 用户正在考虑学校是否应作为域名持有者；学校名称、所在地、校内制度和授权情况尚未提供，不能记为已确定学校持有。
- 网站托管地区和平台尚未选定，注册渠道需要考虑未来部署选择。

## 尚未批准的建议

- 优先考虑 `winet.group`，前提是注册商确认可以注册且具体首年与续费报价被用户接受。
- 内容维护采用“表单录入 → 草稿 → 预览 → 点击发布 → 自动更新”，具体实现与发布权限待架构任务确定。
- 注册域名与购买服务器分开决定。
