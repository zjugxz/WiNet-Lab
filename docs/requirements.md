...
- 使用本地 Git。用户于2026-09-17提出将现有初版提前发布到 zjugxz/WiNet-Lab，新指示替代此前整体验收后才首次上传的时点限制；本轮提供操作指导，未执行推送或启用Pages，不代表整站验收。
- 实时维护项目文档，方便后续 agent 接手。

## 托管与账号（2026-09-15 变更）

2026-09-17最新补充：用户提供 https://github.com/zjugxz/WiNet-Lab ，作为初版发布目标，并询问操作步骤。预期默认项目地址为 https://zjugxz.github.io/WiNet-Lab/ 。由于本轮外部连接失败，仓库可见性、是否为空、权限和Pages状态仍未独立核验。已整理 [发布步骤](github-pages-preview.md)，本轮未实施部署配置或远程操作。下列未提供仓库链接及禁止提前上传等内容是历史状态，以此补充为准。
