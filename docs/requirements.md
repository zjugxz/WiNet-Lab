...
最新Cite扩展指示（2026-09-22）：用户要求Publications页所有已经发表的论文提供Cite功能，效果和按钮图标与Research页完全一致，且必须对每一篇论文访问其网站取得真实可用的cite信息。据此本地实现73条（除无法核实出版的p03）Text/BibTeX/RIS引用，资料经DOI内容协商与官方网站逐篇取得；本地实现，不自动发布，见[验证记录](verification/publications-cite-2026-09-22.md)。
...
- 2026-09-22用户要求新增同级栏目Gallery放置团建活动图片，参考 tns.thss.tsinghua.edu.cn/sun/publication.html 的分组照片墙+点击放大做法；图片后续提供，先实现数据接口、页面、灯箱与空状态。本地实现，不自动发布。
