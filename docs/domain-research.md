...
**历史存档：用户于 2026-09-15 明确取消购买域名，改为评估 GitHub Pages 默认地址。以下查询结果和购买步骤均为历史记录，不再执行；没有注册域名或付款。当前任务见 `progress.md`。**

查询日期：2026-09-15（Asia/Shanghai）。动态状态和价格须在购买前再次核对。

## 历史建议及当时结论

当前重点候选为用户提出并在阿里云查询的 `winetgroup.tech`；`winet.group` 保留作对比。用户已提供查询结果截图，尚未最终确认域名与费用，也未下单。

| 候选 | 本次查询结果 | 结论 |
| --- | --- | --- |
| `winet.group` | Identity Digital 注册局 RDAP 返回 HTTP 404 | 未找到注册记录；需注册商确认开放注册、是否溢价及报价 |
| `winetgroup.tech` | 用户提供的阿里云查询截图显示完整名称、“完美匹配”和“立即注册”，首年人民币 8 元、续费 259 元/年 | 查询页显示可进入注册流程；最终可注册状态及成交金额以提交订单时为准，尚未购买 |
...
证据：2026-09-15 本轮用户上传的阿里云查询截图。核对到域名为 `winetgroup.tech`，首年价人民币 8.00 元，续费人民币 259.00 元/年，单域名行显示“立即注册”。截图未见该名称的明确精品/溢价标记，结算时仍须核对。

- 首年：人民币 8 元。
- 以当前续费价计算，首年加两次续费的三年费用为 `8 + 259 × 2 = 526` 元。
- 首年加四次续费的五年费用为 `8 + 259 × 4 = 1044` 元。
- 以上是按当前价格作的预算估算，不是一次购买多年报价，也不是未来续费锁价。
- 截图下方人民币 89 元及 15 元的组合优惠包含其他后缀，不能与上方单域名价格混为一谈；当前只讨论一个主域名。
- 用户提供截图表示提供核查证据，不等于接受续费成本或授权购买。当前等待其确认具体域名与费用。
...
此前自动抓取未取得阿里云具体名称报价；现已通过用户截图核查 `winetgroup.tech` 的查询页报价，见上节。`.group` 仍未取得阿里云具体报价。最终以具体订单价格为准。
...
1. 查询与截图核对已完成，无需要求用户重复查询。
2. 等待用户确认是否选择 `winetgroup.tech`，并接受查询页首年 8 元、当前续费 259 元/年的费用。
3. 确认后指导选择截图最上方该域名的单独注册入口，再核对注册主体、账号、年限与订单。下方组合优惠不属于本次选定范围。
4. 订单提交前复核实际域名、金额、注册年限及附加项目；当前未进入付款。

后续指导应确认账号由课题组可长期控制、域名持有人符合学校或课题组安排、注册年限及续费设置符合用户意愿。支付由用户完成或另行明确授权；当前没有付款授权。

## 域名持有者与学校主体

用户进一步询问实验室网站是否应以学校为所属方。以下为条件性建议，不是学校授权或用户的最终选择。

- 如果网站是学校认可并长期由学校管理的实验室官网，建议按学校制度，以学校或实际主办单位作为域名持有者。应先取得学校相应授权并准备真实的单位认证材料，不能只填写学校名称而使用不匹配的个人证件。
- 阿里云企业/组织模板要求持有者单位名称与上传证件完全一致，支持事业单位法人证书等机构证件。学院或课题组名称能否作为持有人，应取决于是否具备对应的有效主体证明，不能仅根据展示名称决定。
- 域名持有者、域名联系人、购买时使用的阿里云账号是不同概念。联系人可以是指定老师或管理员；账号实名认证与域名实名认证并非同一事项。实际管理账号及交接安排应遵循学校制度。
- 若未来在中国大陆以学校为主办单位备案，优先保持域名实名主体与主办学校一致；单位备案部分省份允许的例外需要按具体地区核对，不能默认导师或学生的个人实名满足学校备案要求。
- 如果学校要求使用统一管理账号或校内域名，应按校方规定办理。目前没有学校名称、所在地或授权情况，尚不能核查校内具体要求，也不能断言学校已同意持有该域名。
- 下一步应先明确学校名称及是否已获允许以学校名义注册、能否提供单位认证资料；暂不进行主体资料填写或付款。
...
- 阿里云域名信息模板及实名认证：https://help.aliyun.com/zh/dws/user-guide/how-to-complete-domain-name-authentication
- 阿里云备案前期准备：https://help.aliyun.com/zh/icp-filing/basic-icp-service/user-guide/overview
