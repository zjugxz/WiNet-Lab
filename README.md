...
- People: 16 members (1 PI, 8 Ph.D., 7 Master). The PI block shows the bio inline; every card opens a dimmed-overlay profile dialog (native `<dialog>`), with static `/people/{id}/` pages as the no-JS fallback. The PI bio keeps the supplied mixed Chinese/English text verbatim. Optimized WebP photos. Local only, not yet published.
